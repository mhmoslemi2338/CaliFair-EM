...
def train(model, train_set, optimizer, scheduler=None, batch_size=32):
    ...
    for i, batch in enumerate(iterator):
        ...
        logits, y, _ = model(xs.to('cuda'), y.to('cuda'), masks.to('cuda'))
...
def initialize_and_train(trainset, validset, testset, attr_num, args, run_tag):
    ...
    while epoch <= args['n_epochs']:
        start = time.time()
        train(model, trainset, optimizer, scheduler=scheduler,batch_size=args['batch_size'])
        print("train time: ", time.time()-start)

        print(f"=========eval at epoch={epoch}=========")
        dev_f1 = eval_on_task(epoch, model, valid_iter,run_tag)
        ...
        if args['save_model']:
            ...
            if dev_f1 > best_dev_f1:
                best_dev_f1 = dev_f1
                print(args['model_path']+'/'+'model.pt')
                torch.save(model.state_dict(), args['model_path']+'/'+'model.pt')
            else:
                epochs_without_improvement +=1

        if epochs_without_improvement >= 15:
            print("Stopping training due to no improvement in 15 consecutive epochs.")
            break  # Stop training
        epoch += 1
