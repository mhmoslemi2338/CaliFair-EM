...
class DittoModel(nn.Module):
    ...
    def __init__(self, device='cuda', lm='roberta', alpha_aug=0.8):
        ...
        # self.bert = AutoModel.from_pretrained('/home/mmoslem3/DITTO/ditto_light/distilbert-base-uncased-AUTOMODEL')
...
def evaluate(model, iterator, threshold=None):
    ...
    all_y = []
    all_probs = []
    log_freq = 10  # Or set based on your preference
    # pbar = pyprind.ProgBar(len(iterator) // log_freq, bar_char='█', width=30)
    ...
    with torch.no_grad():
        for i, batch in enumerate(iterator):
            ...
            # if i % log_freq == 0:  # Update progress bar at specified frequency
            #     pbar.update()
...
def train_step(train_iter, model, optimizer, scheduler, hp):
    ...
    log_freq = 10  # Frequency of logging and progress bar update

    # Initialize the progress bar
    pbar = pyprind.ProgBar(len(train_iter) // log_freq, bar_char='█', width=30)
